...
class TestCorptoolsCharAccessPerms(OIDCTestCase):
    ...
    def test_no_perms_oauth_u1(self):
        ...
        self.assertIn(
            "External OAuth Denied", response.content.decode("utf-8")
        )
    ...
    def test_with_perms_oauth_u1_all_scopes(self):
        ...
        self.assertIn("openid", response.content.decode("utf-8"))
        self.assertIn("email", response.content.decode("utf-8"))
        self.assertIn("profile", response.content.decode("utf-8"))
    ...
    def test_with_perms_oauth_u1_email_only(self):
        ...
        response = self.client.get("/o/authorize/", data=data)
        self.assertIn(self.oauth_app.name, response.content.decode("utf-8"))
        self.assertNotIn("openid", response.content.decode("utf-8"))
        self.assertIn("email", response.content.decode("utf-8"))
        self.assertNotIn("profile", response.content.decode("utf-8"))
    ...
    def test_full_chain_u1_with_perms_and_wrong_state(self):
        ...
        response = self.client.get("/o/authorize/", data=data)
        self.assertIn(
            f"{self.oauth_app} Access Denied", response.content.decode("utf-8")
        )
        self.assertEqual(200, response.status_code)
